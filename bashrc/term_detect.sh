term_detect() {
    ...
    # documentation
    local docstr="Try to detect current terminal emulator, and set TERM_PROGRAM"
    ...
    [[ -n ${TERM_PROGRAM:-} || -n ${SSH_CLIENT:-} ]] && return
    ...
    local pcmd
    pcmd=$(ps -o 'command=' -ww -p $(ps -o 'ppid=' -p $$))

    if [[ $pcmd =~ bin/vshd\  ]] && [[ $BROWSER == /usr/bin/garcon-url-handler ]]
    then
